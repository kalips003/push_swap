...
void	function2(t_data *data, int i, int sw)
{
	...
	while (--i >= 0)
		temp = temp->below;
	put("(num: %d)[index: %d]\n\n\n", temp->num, temp->num_index);
	fill_stru_algo(data, &algo, temp);
...
void	function3(t_data *data, int sw)
{
	...
	small = assign_str_all(data);
	if (small)
		put("Smallest string: (%c%d) %s\n\n", small->pile_c, small->num, small->algo);
	else
		put("No string: (%p)\n\n", small);
	i = -1;
	temp = data->top_a;
	while (++i < data->full_size)
	{
		if (i == data->size_a && put("\n"))
			temp = data->top_b;
		put("%c) -%d- = %s\n", temp->pile_c, temp->num, temp->algo);
